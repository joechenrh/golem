...
| **Tool** | `tools.Tool` | Shell, file ops, web search/fetch/HTTP, Lark docs, scheduler, spawn_agent, skills, external plugins (e.g. mem9 memory) |
...
**Tools and Sessions.** `BuildToolRegistry` registers core tools (shell, file ops), web tools, Lark tools, persona memory, discovered skills, and external plugins from `~/.golem/plugins/` (including optional memory tools like mem9). Two middlewares wrap every tool call: `CacheMiddleware` (60s TTL for read-only tools) and `Redact` (strips secrets from output). A schedule store backed by `~/.golem/agents/<name>/schedules.json` provides `schedule_add`, `schedule_list`, and `schedule_remove` tools. The `spawn_agent` tool is registered only on the top-level registry; sub-agents get their own tape, tool registry, and hooks but share the LLM client, and their registries intentionally omit `spawn_agent` to prevent recursive spawning. In top-level sessions, `spawn_agent` is **asynchronous**: it launches sub-agents as background goroutines via a `TaskTracker` (backed by `errgroup` with a concurrency limit of 5) and returns immediately with a task ID. Completed task results are automatically delivered to the session as ephemeral messages at the top of each ReAct iteration via an in-memory wait mechanism — no polling is required. In sub-sessions (which have no tracker), `spawn_agent` falls back to synchronous execution. A default `Session` handles CLI input, while a `SessionManager` is created when the agent has remote channels. The session manager gives each remote chat ID an isolated session with its own tape and tool registry, and restores existing sessions from tape files matching `session-<agent>-<chatID>-<ts>.jsonl`.
