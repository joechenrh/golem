...
func TestRegistry_Hide_ExcludesFromDefinitions(t *testing.T) {
	r := NewRegistry()
	r.Register(newMockTool("read_file", "Read", "Read full"))
	r.Register(newMockTool("web_search", "Search", "Search full"))
	r.Hide("web_search")

	defs := r.ToolDefinitions()
	if len(defs) != 1 {
		t.Fatalf("len(defs) = %d, want 1 (hidden tool excluded)", len(defs))
	}
	if defs[0].Name != "read_file" {
		t.Errorf("defs[0].Name = %q, want read_file", defs[0].Name)
	}
}

func TestRegistry_Hide_StillExecutable(t *testing.T) {
	r := NewRegistry()
	r.Register(newMockTool("web_search", "Search", "Search full"))
	r.Hide("web_search")

	result, err := r.Execute(context.Background(), "web_search", `{}`)
	if err != nil {
		t.Fatalf("Execute() error: %v", err)
	}
	if result != "mock result for web_search" {
		t.Errorf("result = %q", result)
	}
}

func TestRegistry_Expand_UnhidesHiddenTool(t *testing.T) {
	r := NewRegistry()
	r.Register(newMockTool("web_search", "Search", "Search full"))
	r.Hide("web_search")
	r.Expand("web_search")
	...
	if defs[0].Description != "Search full" {
		t.Errorf("should be expanded with full desc, got %q", defs[0].Description)
	}
}

func TestRegistry_UnhideHints_FragmentMatch(t *testing.T) {
	r := NewRegistry()
	r.Register(newMockTool("web_search", "Search", "Search full"))
	r.Register(newMockTool("http_request", "HTTP", "HTTP full"))
	r.Register(newMockTool("read_file", "Read", "Read full"))
	r.Hide("web_search")
	r.Hide("http_request")

	// "search" (4+ chars) should match "web_search" fragment.
	unhidden := r.UnhideHints("please search for something")
	if len(unhidden) != 1 || unhidden[0] != "web_search" {
		t.Errorf("unhidden = %v, want [web_search]", unhidden)
	}

	// web_search should now appear in definitions (compact).
	defs := r.ToolDefinitions()
	found := false
	for _, d := range defs {
		if d.Name == "web_search" {
			...
			if d.Description != "Search" {
				t.Errorf("unhidden tool should be compact, got %q", d.Description)
			}
		}
	}
	if !found {
		t.Error("web_search should appear in definitions after unhiding")
	}
}

func TestRegistry_ShrinkUnused_RehidesDefaultHidden(t *testing.T) {
	r := NewRegistry()
	r.Register(newMockTool("web_search", "Search", "Search full"))
	r.Register(newMockTool("read_file", "Read", "Read full"))
	r.Hide("web_search")

	// Expand+unhide via ExpandAt (simulates LLM calling the tool).
	r.ExpandAt("web_search", 0)
	r.ExpandAt("read_file", 0)

	// After stale period, web_search should be re-hidden, read_file just compacted.
	r.ShrinkUnused(5, 3)

	defs := r.ToolDefinitions()
	for _, d := range defs {
		if d.Name == "web_search" {
			t.Error("web_search should be re-hidden after stale period")
		}
		if d.Name == "read_file" && d.Description != "Read" {
			t.Errorf("read_file should be compacted, got %q", d.Description)
		}
	}
}
